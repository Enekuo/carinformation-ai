...
import LanguageSwitcher from "@/components/LanguageSwitcher"; // ← TU SWITCHER REAL DE EUSKALIA
            ...
            >
              <div className="h-full flex flex-col justify-between">
                <nav className="space-y-1">
                  <Link to="/dashboard" className={`w-full flex items-center gap-3 h-11 ps-2 pe-2 rounded-xl ${navHoverBg}`} style={isActive("/dashboard") ? { backgroundColor: ACTIVE_BG_COLOR } : undefined}>
                    <Home className="w-5 h-5 shrink-0" />
                    <span className="truncate">{t("dashboard_nav_home")}</span>
                  </Link>

                  <Link to="/create" className={`w-full flex items-center gap-3 h-11 ps-2 pe-2 rounded-xl ${navHoverBg}`} style={isActive("/create") ? { backgroundColor: ACTIVE_BG_COLOR } : undefined}>
                    <PlusCircle className="w-5 h-5 shrink-0" />
                    <span className="truncate">{t("dashboard_nav_create")}</span>
                  </Link>

                  <Link to="/library" className={`w-full flex items-center gap-3 h-11 ps-2 pe-2 rounded-xl ${navHoverBg}`} style={isActive("/library") ? { backgroundColor: ACTIVE_BG_COLOR } : undefined}>
                    <Folder className="w-5 h-5 shrink-0" />
                    <span className="truncate">{t("dashboard_nav_library")}</span>
                  </Link>

                  <Link to="/assistant" className={`w-full flex items-center gap-3 h-11 ps-2 pe-2 rounded-xl ${navHoverBg}`} style={isActive("/assistant") ? { backgroundColor: ACTIVE_BG_COLOR } : undefined}>
                    <MessageSquare className="w-5 h-5 shrink-0" />
                    <span className="truncate">{t("dashboard_nav_ai_chat")}</span>
                  </Link>

                  <Link to="/pricing" className={`w-full flex items-center gap-3 h-11 ps-2 pe-2 rounded-xl ${navHoverBg}`} style={isActive("/pricing") ? { backgroundColor: ACTIVE_BG_COLOR } : undefined}>
                ...
                <div className="pb-0">
                  <Link to="/settings" className={`w-full flex items-center gap-3 h-11 ps-2 pe-2 rounded-xl ${navHoverBg}`} style={isActive("/settings") ? { backgroundColor: ACTIVE_BG_COLOR } : undefined}>
            ...
            <div className="w-full bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100">
              <div className="mx-auto max-w-5xl px-6 md:px-10 lg:px-12 py-8 md:py-12">
                ...
                <div className="mb-6 md:mb-8">
                  ...
                  <p className="mt-1 text-slate-600 dark:text-slate-400">{t("settings_subtitle")}</p>
                </div>

                <form onSubmit={saveAll} className="space-y-6 md:space-y-8">

                  {/* PERFIL */}
                  <Section title={t("settings_profile_title")} description={t("settings_profile_desc")}>
                    <Row>
                      <Field label={t("settings_profile_display_name")}>
                        <input
                          ...
                          placeholder={t("settings_profile_display_name_ph")}
                        />
                      </Field>
                    ...
                    <div className="flex items-center justify-between gap-3 pt-2">
                      ...
                      >
                        {t("settings_cta_manage_plan")}
                      </Link>
                    </div>
                  </Section>

                  {/* APARIENCIA */}
                  <Section title={t("settings_appearance_title")} description={t("settings_appearance_desc")}>
                    <Row>
                      ...
                      {/* SELECTOR DE IDIOMAS DE EUSKALIA */}
                      <Field label={t("settings_appearance_language")} hint={t("settings_appearance_language_hint")}>
                        <div className="rounded-lg border border-slate-200 dark:border-slate-800 p-2 inline-flex">
                          <LanguageSwitcher />
                        </div>
                      </Field>
                    </Row>
                  ...
                  {/* NOTIFICACIONES */}
                  <Section title={t("settings_notifications_title")} description={t("settings_notifications_desc")}>
                    <div className="space-y-3">
                      <label className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400">{t("settings_notifications_product_hint")}</p>
                        </div>
                      </label>
                      ...
                      <label className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400">{t("settings_notifications_tips_hint")}</p>
                        </div>
                      </label>
                      ...
                      <label className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400">{t("settings_notifications_billing_hint")}</p>
                        </div>
                      </label>
                    </div>
                  </Section>
