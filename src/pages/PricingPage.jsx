...
export default function PricingPage() {
  ...
  // ✅ Usa fallback si t(key) === key o está vacío
  const tr = (k, f) => {
    const v = t(k);
    return !v || v === k ? f : v;
  };
  ...
  const plans = [
    {
      ...
      features: [
        ...
        tr("pricing.features.audio_free", "Audio (TTS) limitado"),
    ...
    },
    {
      ...
      features: [
        ...
        tr("pricing.features.audio_basic", "Audio 200 min/mes"),
    ...
    },
    {
      ...
      features: [
        ...
        tr("pricing.features.audio_premium", "Audio 1.000 min/mes"),
    ...
    <main className="min-h-[calc(100vh-4rem)] flex flex-col items-center relative bg-gradient-to-br from-slate-100 via-sky-50 to-blue-100 p-6 pt-12 md:pt-20">
      ...
      <div className="text-center mb-10 md:mb-16">
        ...
        <p className="text-lg text-slate-600 max-w-3xl mx-auto">
          {tr("pricing.subtitle", "Desbloquea todo el potencial con el plan que mejor se adapte a ti.")}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-6xl">
        {plans.map((plan) => (
          <div
            key={plan.id}
            className={`group relative bg-white p-8 rounded-xl shadow-xl border-2 ${plan.borderColor} flex flex-col transition-transform duration-300 hover:shadow-2xl hover:-translate-y-1 hover:scale-[1.01] ${plan.glow ? "shadow-primary/30" : ""}`}
          >
            {plan.glow && (
            ...
            <div className="relative z-10 flex flex-col h-full">
              ...
              <div className="text-center mb-6">
                <p className="text-4xl font-extrabold">
                  ...
                  {plan.priceSuffixKey && (
                    <span className="text-base font-normal text-slate-500"> {tr(plan.priceSuffixKey, "al mes")}</span>
                  )}
                </p>
                {plan.perDayKey && <p className="text-xs text-slate-500 mt-1">{plan.perDayKey}</p>}
              </div>

              <ul className="space-y-4 mb-8 flex-grow">
                {plan.features.map((f, i) => (
                  <li key={i} className="flex items-start">
                    <CheckIcon className={`h-5 w-5 mr-2 mt-0.5 flex-shrink-0 ${plan.checkColorClass}`} />
